...
#
# Two surfaces over the C SAX API (taurus/sax/sax.h):
#
#   Taurus::SAX.parse(xml, handlers)   — one-shot parse of a String.
#   Taurus::SAX::Parser                — incremental feed() for streams.
#
...
module Taurus
  module SAX
    # C struct TaurusSAXHandler — 11 function pointers, same order
    # as sax.h.
    class HandlerStruct < FFI::Struct
      layout(
        :start_document,         Taurus.find_type(:sax_start_document),
        :end_document,           Taurus.find_type(:sax_end_document),
        :start_element,          Taurus.find_type(:sax_start_element),
        :end_element,            Taurus.find_type(:sax_end_element),
        :characters,             Taurus.find_type(:sax_characters),
        :comment,                Taurus.find_type(:sax_comment),
        :cdata,                  Taurus.find_type(:sax_cdata),
        :processing_instruction, Taurus.find_type(:sax_pi),
        :start_prefix_mapping,   Taurus.find_type(:sax_prefix_mapping),
        :end_prefix_mapping,     Taurus.find_type(:sax_end_prefix),
        :error,                  Taurus.find_type(:sax_error)
      )
    end
    ...
    class Bridge
      ...
      # The struct pointer to hand to C as TaurusSAXHandler*.
      def pointer
        @struct
      end
    ...
    #
    # Returns true on success. Raises Taurus::Error when parsing
    ...
    class Parser
      ...
      def free
        ...
        Taurus.taurus_sax_parser_free(@parser)
        @parser = nil
        @bridge.release
      end
    end
