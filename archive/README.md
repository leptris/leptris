...
active build** — see `src/CMakeLists.txt` `LEPTRIS_SOURCES` for the
authoritative list of files that are compiled into libleptris.
...
│                              src/leptris/parse/parser_new.c.
│   ├── leptris_parse.c
│   ├── parse_content.c
│   ├── parse_document.c
│   ├── parse_simple.c
...
1. **Disabled at the CMake level** (commented out of `LEPTRIS_SOURCES`).
...
`LEPTRIS_SOURCES` in `src/CMakeLists.txt`.  The file's git history will
follow naturally.
