/* leptris_parse.c - XML parser core helper functions
...
int leptris_parse_context_init(LeptrisParseContext *ctx,
    ...
    if (opts) {
        ...
        leptris_parse_options_init(&ctx->opts);
    }
 ...
 *       Caller must copy with leptris_strndup() if needed beyond parse.
 */
const char *parse_name(LeptrisParseContext *ctx, size_t *len) {
    ...
    while (ctx->pos < ctx->end && leptris_is_name_char(*ctx->pos)) {
        ctx->pos++;
    }
 ...
 *
 * NOTE: Returned pointer is into parse buffer, NOT null-terminated.
 *       Caller must copy with leptris_strndup() if needed.
 */
