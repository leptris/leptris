...
TEST(Qt3Subset, FnSubstring) {
    std::string xml = slurp(std::string(LEPTRIS_QT3_DIR) + "/fn/substring.xml");
    ASSERT_FALSE(xml.empty());
    LeptrisStatus st = LEPTRIS_OK;
    LeptrisDocument ts = leptris_parse_string(xml.data(), xml.size(), &st);
    ASSERT_NE(ts, nullptr);

    /* The single concepts-env case uses ./concepts/@id from its
     * source document as context. */
    std::string concepts = slurp(std::string(LEPTRIS_QT3_DIR) +
                                 "/fn/substring/concepts.xml");
    LeptrisDocument cdoc =
        concepts.empty()
            ? NULL
            : leptris_parse_string(concepts.data(), concepts.size(), &st);
    ...
    /* Exact gate: fn/substring has 48 test-cases; the adopted
     * set (supported assertions, supported environments) is 45
     * (1 assert-type case, 2 others skip). */
    EXPECT_EQ(agree, run);
    EXPECT_EQ(run, 45);
    if (cdoc) leptris_document_free(cdoc);
