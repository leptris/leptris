...
**libtaurus** — pure C99 XML 1.0 parser, XPath 1.0 engine (W3C-conformant), and SAX parser. Ships as a static/shared library plus a `taurus` CLI. Zero required runtime deps; `utf8proc` (Unicode) and `iconv` (encoding conversion) are optional.
...
- `TAURUS_BUILD_CLI` (ON) — builds the `taurus` executable.
- `TAURUS_BUILD_BENCHMARKS` (OFF) — enables libxml2/pugixml comparison benchmarks under `benchmarks/`.
- `TAURUS_BUILD_MAN_PAGES` (OFF) — generates man pages from `.adoc` via `cmake/modules/AdocMan.cmake`.
- `TAURUS_ENABLE_UTF8PROC` / `TAURUS_ENABLE_ICONV` (ON/ON) — toggle Unicode/encoding features. Disabling compiles those `.c` files out via `TAURUS_HAS_*` macros.
- `TAURUS_BUILD_STATIC` (ON) / `TAURUS_BUILD_SHARED` (OFF).

### Useful targets
- `taurus-cli` (output name `taurus`) — the CLI binary under `build/cli/`.
- `taurus_static` / `taurus_shared` / alias `taurus::taurus`.
...
Public API (include/) →  taurus_parse_*, taurus_xpath_eval, taurus_document_*, ...
Core (src/taurus/)    →  DOM, parser, XPath engine, SAX, DTD, encoding, memory
```

**The CLI never touches XML structures directly — it always goes through the public API in `src/include/taurus/`.** See `cli/CLI_ARCHITECTURE.md` for the design contract (MECE options, Open/Closed command registration, no `#ifdef` code guards — solve platform differences architecturally).

### Core subsystems (`src/taurus/`)
- `dom/` — node tree: `node`, `element`, `text`, `comment`, `cdata`, `pi`, `doctype`. **Compact architecture**: elements are ~96 bytes via compressed pointers; all node types begin with `TaurusNode` so they're safely castable.
- `parse/` — `parser_new.c` is the active parser; `compact_parser.c` is the compact-mode path. (Legacy `taurus_parse.c` / `parse_*.c` files at the subsystem root are older implementations — confirm which is wired in via `src/CMakeLists.txt` before editing.)
...
- `encoding/` — UTF-16 always compiled; `encoding.c` (iconv) only when `TAURUS_ENABLE_ICONV` is on.
- `unicode/unicode.c` — only compiled when `TAURUS_ENABLE_UTF8PROC` is on.
...
### Public API surface (`src/include/taurus/`)
The public contract. Don't break ABI without a major bump. Headers split by subsystem: `dom/{document,element,serialize}.h`, `xpath/xpath.h`, `sax/sax.h`, plus `types.h`, `error.h`, `dtd.h` at the top level. All handles are opaque typedefs (`TaurusDocument`, `TaurusElement`, `TaurusNodeRef`, …); callers never see struct definitions.

### Memory model
- Pool allocator is the dominant allocation path — node creation should go through `taurus_node_create_pooled`.
- Strings are stored as `StringView` (`common/string_view.{h,c}`) for zero-copy slicing during parse.
- No reference counting; ownership is document-scoped — free via `taurus_document_free`.
...
- Public types live in `src/include/taurus/types.h` (single canonical source); `taurus.h` includes it and adds the function declarations. See TODO 99.
- Internal headers re-include `taurus/types.h` via `taurus_internal.h` → `memory/pool.h`; do not redeclare public typedefs locally.
- Benchmarks are C++11.
- Memory ownership in the public API is documented per-function in the headers (look for "Memory:" comments) — preserve these contracts. Strings returned as `const char*` from a `TaurusDocument`-taking accessor are document-owned and live until `taurus_document_free`.
- Error reporting goes through `TaurusStatus` codes (`src/taurus/error.c`); the CLI wraps these via `cli/error.{h,c}`. Public-API entry points must write only public `TaurusStatus` constants to their `TaurusStatus*` out-params — the internal `taurus_error_code` enum is for `error.c`'s thread-local channel only (TODO 98).
...
- Man pages live under `cli/man/*.1.adoc` and `src/man/libtaurus.5.adoc` and are only generated when `TAURUS_BUILD_MAN_PAGES=ON`.
