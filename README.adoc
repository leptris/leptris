...
specialization, predicate fast paths, and absolute-path fusion
(TODO 120, TODO 125–129). Numbers below are from
...
Per-call floor (1 µs) is at libxml2 parity after TODO 125 (lazy
namespace init). The remaining gap is on subtree traversal
(descendant / `//`), where the recursive walk cost dominates.
Function-wrapped paths (`count(//book)`) are slower still because
the function-arg path uses the AST evaluator instead of the VM —
pre-evaluating args via the VM is future work.
...
* XPath function-arg pre-evaluation — make `count(//foo)`,
  `name(//foo)`, etc. use the bytecode VM for the path argument
  instead of the AST evaluator. Closes the function-wrapped gap.
* Iterative subtree walk + pre-allocated result capacity for
  descendant axis. Closes the remaining 5× gap on `descendant::*`.
