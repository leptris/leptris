...
`src/leptris/dom/element.c:557`:

```c
/* TODO: Implement proper namespace list */
```

`leptris_element_add_namespace_inplace` set `elem->prefix` and
`elem->namespace_uri` directly but did NOT add an entry to
`elem->namespaces`. That meant `leptris_element_lookup_namespace`
could not find namespaces registered through this path.

## Resolution (PR #32)

`leptris_element_add_namespace_inplace` now does both:

1. Sets `elem->prefix` / `elem->namespace_uri` (zero-copy direct
   fields, as before).
2. When a pool is provided, allocates a `struct leptris_namespace`
   via `leptris_namespace_new_pooled` and adds it to
   `elem->namespaces` via `leptris_element_add_namespace`.
...
regular `leptris_element_add_namespace` + `leptris_namespace_new_pooled`
