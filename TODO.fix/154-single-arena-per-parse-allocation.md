...
2. `taurus_alloc_hook(sizeof(TaurusMemoryPool))` for the pool struct.
3. `taurus_alloc_hook(page_size + sizeof(MemoryPage))` for the first
   pool page (called from `taurus_pool_create_with_page_size`).
4. `calloc(1, sizeof(struct taurus_document))` for the doc struct.
...
- New internal API: `taurus_pool_create_combined(page_size)` returns
  a pool whose `TaurusMemoryPool` struct and first `MemoryPage` live
  in the same allocation.
- The pool struct goes at offset 0; the page header follows at
  `sizeof(TaurusMemoryPool)`; the page data follows.
- `taurus_pool_destroy` must know that the first page is not a
...
- `direct_parse_internal` allocates the doc via `taurus_pool_alloc`
  instead of `calloc`.
- Need a `doc_pool_allocated` flag on the doc struct so
  `taurus_document_free` knows whether to `TAURUS_FREE(doc)` or
  leave it to be reclaimed by `taurus_pool_destroy`.
- All other doc-allocation sites (`taurus_document_copy`,
  `taurus_parse_fragment`) keep using `calloc` for now — they're not
  hot paths.

### Phase C — Optional: skip the buf copy on the writable fast path
- `taurus_parse_string` could expose an `adopt_buffer` mode where
  the caller's `char*` is used directly (no malloc+memcpy).
- Already exposed as `taurus_parse_string_inplace`. The benchmark
  shows this saves ~100 ns on tiny docs.
- Document the contract: caller must NOT free until doc is freed.

## Risk

- **ABI**: `taurus_document` size grows by 1 byte (flag). Not visible
  to callers (opaque handle).
- **Free ordering**: pool-allocated doc must survive until
  `taurus_pool_destroy` runs in `taurus_document_free`. Already true
