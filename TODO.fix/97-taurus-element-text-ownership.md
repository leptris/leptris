# TODO 97 — taurus_element_text ownership contract violation
...
4 bytes via `taurus_element_text_int` → `taurus_element_text`).

## Symptom
Every call to `taurus_element_text()` malloc'd a buffer and returned it as
...
Direct leak of 4 byte(s) in 1 object(s) allocated from:
  taurus_element_text_int  src/taurus/dom/element_query.c:664
  → taurus_element_text    src/taurus/dom/element_query.c:46
  → taurus_element_get_text_content  src/taurus/dom/element.c:854 (taurus_malloc)
```
...
1. **Single text/CDATA child** — return the child node's own storage
   directly (`TaurusTextNode::content` / `TaurusCDATANode::content`).
   Zero allocation, no leak possible.
2. **Mixed content** — concatenate via the existing
   `taurus_element_get_text_content`, then intern the result in the
   document pool via `taurus_pool_strdup`. The string is released when
   `taurus_document_free` destroys the pool.
...
`taurus_malloc` would re-introduce the leak).
...
* `TextIsDocumentOwnedForMixedContent` — mixed content → concatenated
  into document pool, valid until `taurus_document_free`.
...
`taurus_attribute`, `taurus_node_name` — all return borrowed pointers).
...
* Header docstring updated in `src/include/taurus.h` and
  `src/include/taurus/dom/element.h` to state the document lifetime
