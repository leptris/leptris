...
**Status**: design — `leptris_node_thaw` is a stub

## What's in place

Public freeze API (shipped earlier this session):

* `leptris_document_freeze(doc)` — freezes the entire tree
* `leptris_document_is_frozen(doc)` — checks
* `leptris_node_freeze(node)` — recursive
* `leptris_node_is_frozen(node)` — checks

The parser calls `leptris_document_freeze_tree` internally so
every freshly-parsed document is frozen.

## What's broken

`leptris_node_thaw` in `src/leptris/dom/node.c` is a TODO stub:

```c
LeptrisNode* leptris_node_thaw(LeptrisNode* node) {
    if (!node) return NULL;
...
`leptris_document_clone` API. `leptris_node_thaw` is removed.
...
existing API surface (`leptris_element_set_attribute(elem, ...)`
returns `LeptrisStatus`, not a new `elem`) doesn't naturally
...
1. The public API surface (`set_attribute`, `set_text`,
   `append_child`, …) returns `LeptrisStatus`, not a new element
...
2. Remove `leptris_node_thaw` from `dom/node.h` (internal header,
   no callers).
3. Delete the `leptris_node_thaw` implementation.
...
5. If a real COW use case emerges later, design it as an explicit
   `leptris_document_clone` API rather than transparent COW.
