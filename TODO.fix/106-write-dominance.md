...
| Operation | leptris | pugixml | libxml2 | leptris vs pugixml | leptris vs libxml2 |
|---|---|---|---|---|---|
...
pugixml beats both leptris AND libxml2 by huge margins on every write
operation. Leptris is roughly competitive with libxml2 — faster on
append, slower on attrs.

## Why pugixml wins

Architectural advantages pugixml has over both leptris and libxml2:

1. **Compact storage**: pugixml packs a node into 44 bytes (vs
   leptris's ~96).  Better cache locality, fewer allocations per page.
2. **Custom slab allocator**: no per-node pool call. Bulk-allocates
   pages, doles out from a free list. Cheaper than leptris's pool.
3. **No StringView conversion**: pugixml stores char* directly into
   a string buffer that grows in chunks.  Leptris pays pool_strdup
   per name + per attribute name + per attribute value.
4. **No attribute hash interning**: leptris's per-attribute intern
   lookup is ~200ns × 100 attrs = 20µs of pure interning overhead.
5. **Inlined mutation paths**: pugixml is a header-only library; the
   compiler sees everything. Leptris's mutation entry points are
...
The `leptris_element_add_attribute` path does eager name interning
via the pool's hash table. For mutation workloads (not parsing),
interning is wasted work — the user knows the attr is unique.

Add a non-interning path: `leptris_element_set_attribute` (the
public mutation API) calls a new `leptris_element_add_attribute_fast`
...
Add `leptris_element_create_batch(doc, name, n)` that pre-allocates
...
Leptris's element struct is ~96 bytes. pugixml's is 44. The gap is
mostly:
- 32 bytes of LeptrisStringView (name, prefix, namespace_uri) per
  element (3 × 16 bytes)
- 24 bytes of cached cstr pointers (name, prefix, namespace_uri)
- Element pointer fields (parent, first/last child, next sibling)

pugixml uses compact pointer encoding (similar to leptris's compact
mode that's currently unused).  Could reduce leptris's element to
...
leptris's pool uses 32KB pages by default. pugixml uses 4KB-32KB
...
- **Set 100 attrs**: leptris within 2x of pugixml (was 7x; Phase 1 + 2)
- **Append 1000 children**: leptris within 2x of pugixml (was 4x; Phase 2)
