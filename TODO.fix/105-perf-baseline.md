...
Until PR #66 the benchmark suite claimed to compare leptris vs libxml2
...
| Benchmark | Leptris | libxml2 | Ratio |
|---|---|---|---|
| SAX small | 6.28 µs | 5.39 µs | leptris **1.16x slower** |
| SAX medium | 24.89 µs | 17.44 µs | leptris **1.43x slower** |
...
| Benchmark | Leptris | libxml2 | Ratio |
|---|---|---|---|
| Parse + Root | 119 µs | 47 µs | leptris **2.53x slower** |
| Tree Traversal | 5.45 µs | 1.52 µs | leptris 3.59x slower |
| Attribute Access (100x) | 1.57 µs | 2.98 µs | leptris **1.90x faster** |
| Text Extraction (100x) | 1.35 µs | 3.46 µs | leptris **2.56x faster** |
| Child Iteration (100x) | 10.93 µs | 2.50 µs | leptris 4.37x slower |

DOM access is mixed — leptris wins on attribute access and text
...
Existing bench files (`bench_xpath_leptris`, `bench_xpath_libxml2`, etc.)
...
~235ns per element. The per-element work leptris does that libxml2
doesn't:

1. **Eager name conversion** (`leptris_sv_to_cstr_pooled` at element.c
...
Total addressable: ~38 µs out of 119 µs (32%). That would bring leptris
...
free-list of N pre-allocated leptris_attribute structs. Reward: ~18 µs.
