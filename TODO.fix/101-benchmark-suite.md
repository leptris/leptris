...
2. **Bit-rotted bench files**: `bench_node_creation.c` used the
   ...
   used the pre-rename `leptris_element_text_content`.  None of it built.
3. **Harness gaps**: only wall-clock time.  The user explicitly asked
   for CPU and RAM tracking — without them, "is leptris faster?" was
...
* `leptris_add_benchmark` helper standardizes include + link setup.
...
* Drive-by fixes: `bench_node_creation.c` include paths corrected
  ...
  `leptris_element_text` (renamed); `dom_benchmark_v2` links `utils.c`.

### SAX baseline (new: `benchmarks/sax/`)
* `bench_leptris.c` and `bench_libxml2.c` exercise the SAX reader
...
libxml2 is currently 3-5x faster on SAX, and leptris uses ~35% less
...
* **Modification benchmark vs pugixml** — currently leptris-only.
...
* **XPath coverage** — leptris has 6 bench files in `xpath/` but
