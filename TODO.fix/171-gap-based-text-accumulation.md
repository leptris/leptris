...
**Deferred.** Investigated; for taurus's typical workload (text +
...
taurus's docs are mostly one or the other.
...
2. **Borrowed text is the fast path.** `taurus_text_create_borrowed`
...
4. **The actual hot path is per-attribute, not per-text.** TODO 165
   benchmark shows taurus at ~30 ns/attr vs pugixml at ~8 ns/attr.
...
1. Add a "merge with previous sibling if it's a text node" branch
   in `dp_wire_child` for `TaurusTextNode`.
2. Provide `taurus_text_append(TaurusTextNode*, const char*, size_t)`
...
- Borrowed → owned transition is a memory-ownership change that
  ripples through `taurus_text_free` and `taurus_text_get_content`.
- Edge cases: text + comment + text should NOT merge.

## Expected impact

<2% on bench_dom_taurus (text-heavy docs would see more). Not the
next leverage point.
