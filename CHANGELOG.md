...
- tricky01:8 table scope + marker prune — #659 corpus at ZERO reds (html)
- table starts burst across foreign subtrees (tables01:18) (html)
- comments past </html> are document epilog nodes (tests18:34) (html)
- after-body/after-after-body comment phase routing (webkit01:22-28, tests19:21, tests1:34) (html)
- after-head comments defer; head elements continue into the head (tests3:3) (html)
- numeric CR reference normalizes to LF; after-body comments follow ws into the body (html)
- adoption unlinks the block; table ws runs skip the reconstruct (tests19:95, tricky01:6) (html)
- adoption-append unlinks the furthest block from its old parent (html)
- table-structural end tags burst the foreign scope (namespace-sensitivity:1) (html)
- a nested table inside a cell nests; the cell fences the table-close walk (tests7:6) (html)
- plaintext is raw-to-EOF and still reconstructs formatting (tests18:23, tests19:102) (html)
- stray </head> closes an empty head phase (tests6:1) (html)
- comment continuation lines + #data escape convention (html5libharness)
- foreign roots exit colgroup; dialog special; head-noscript explicit-html variant (html)
- before-head ws drops at the first body token (tricky01:2/4) (html)
- ordinary end tags fence at special elements; colgroup sibling pop + body drop (html)
- select nests in optgroup when no select is open (tests1:35) (html)
- pre entity-newline skip; colgroup any-start exit; form-over-table foster mode (html)
- </body> at top level also implies the document html (html)
- before-head pure-ws drop (guarded), </body> implies html, quote-aware raw close (html)
- four singles - menuitem walk-close, eq-quotes, colgroup split, before-head ws (html)
- marker kinds - table markers transparent to the a-dup in cells (html)
- post-table text does not reconstruct with an open cell (html)
- scope-ignored formatting ends drop their AFE entry; table markers (html)
- </table> closes stack formatting below it; a-dup scope guard (html)
- EOF-in-tag drops the token; no empty raw-text children (html)
- no-table foster fallback applies to HTML rows only (html)
- foreign </svg> scope, template a-nesting, rawtext boundary, doc-start ws (html)
- non-group starts exit a colgroup (foster before the table) (html)
- head-noscript text exits; plaintext closes at </plaintext> (html)
- four content-context ignores from the unsafe corpora (html)
- after-</html> ws flows to the body; <col> drops outside tables (html)
- mid-table <col> reopens a colgroup; list-scope </li> (html)
- unterminated comment drops the trailing dash run at EOF (html)
- frameset vanishes in a template; no-table fostering (html)
- in-head whitespace prefix of mixed text runs + content_len (html)
- <input type=hidden> in a table is not fostered (html)
- a cell start after fostered content joins the open row (html)
- vendored </form> - implied ends, then pop only when current (html)
- select exits, input fostering, sibling tables (html)
- <hr>/<select> closes the open optgroup layers (WHATWG) (html)
- colgroup text foster, caption clears to table, stray </p> after </br> (html)
- corpus expected-parser accepts '<'-named attribute lines (html)
- the end-tag fence exempts only </address> (tests1:26) (html)
- a <select> start opens inside the current option (WHATWG) (html)
- frameset text filter, table-in-p, block fences, foreign br/p pop (html)
- form pointer + in-caption cell reprocess (html)
- structural tag attr merges - first wins, everywhere (html)
- empty-name DOCTYPE is still recorded (13.2.5) (html)
- after-frameset trailing whitespace stays an html child (html)
- frameset text rules; multi-codepoint entities (html)
- qualified HTML names literal; in-select-in-table reprocess; raw-text ignored in select (html)
- after-head whitespace lands between head and body (webkit01:35/36) (html)
- input type=hidden keeps frameset-ok; after-</head> whitespace leaves the head run (html)
- gate the solidus/body/lift changes to the WHATWG mode (html)
- ignore the self-closing solidus on HTML-namespace start tags (html)
- adoption-agency scope check + formatting fallback fence (html)
- scope-fence stray end tags at button/marquee/object/applet (html)
- preserve qualified HTML names and explicit-body frameset semantics (html)
- svg/math namespace roots; head-lift window closes at implied body (html)
- in-select hr, button scope close, HTML end tags pop foreign scope (html)
...
- run the canonical cached bytecode after a twin-compile store (#1079) (xpath)
