...
static const char* encoding_names[] = {
    [TAURUS_ENCODING_UTF8] = "UTF-8",
    [TAURUS_ENCODING_UTF16LE] = "UTF-16LE",
    [TAURUS_ENCODING_UTF16BE] = "UTF-16BE",
    [TAURUS_ENCODING_UTF32LE] = "UTF-32LE",
    [TAURUS_ENCODING_UTF32BE] = "UTF-32BE",
    [TAURUS_ENCODING_ISO8859_1] = "ISO-8859-1",
    [TAURUS_ENCODING_ISO8859_2] = "ISO-8859-2",
    [TAURUS_ENCODING_ISO8859_15] = "ISO-8859-15",
    [TAURUS_ENCODING_WINDOWS_1252] = "WINDOWS-1252",
    [TAURUS_ENCODING_SHIFT_JIS] = "SHIFT_JIS",
    [TAURUS_ENCODING_EUC_JP] = "EUC-JP",
    [TAURUS_ENCODING_GB18030] = "GB18030",
    [TAURUS_ENCODING_EBCDIC_037] = "EBCDIC-CP-US",
    [TAURUS_ENCODING_UNKNOWN] = NULL
};

/**
 * Detect encoding from byte order mark (BOM)
 */
static taurus_encoding_t detect_bom(const unsigned char* data, size_t len) {
    if (len >= 3) {
        /* UTF-8 BOM: EF BB BF */
        if (data[0] == 0xEF && data[1] == 0xBB && data[2] == 0xBF) {
            return TAURUS_ENCODING_UTF8;
        }
    }
    ...
    if (len >= 4) {
        ...
        if (data[0] == 0xFF && data[1] == 0xFE &&
            ...
            return TAURUS_ENCODING_UTF32LE;
        }
        /* UTF-32BE BOM: 00 00 FE FF */
        if (data[0] == 0x00 && data[1] == 0x00 &&
            data[2] == 0xFE && data[3] == 0xFF) {
            return TAURUS_ENCODING_UTF32BE;
        }
    }
    ...
    if (len >= 2) {
        /* UTF-16LE BOM: FF FE */
        if (data[0] == 0xFF && data[1] == 0xFE) {
            return TAURUS_ENCODING_UTF16LE;
        }
        /* UTF-16BE BOM: FE FF */
        if (data[0] == 0xFE && data[1] == 0xFF) {
            return TAURUS_ENCODING_UTF16BE;
        }
    }

    return TAURUS_ENCODING_UNKNOWN;
}

/**
 * Heuristic detection for common encodings
 */
static taurus_encoding_t detect_heuristic(const unsigned char* data, size_t len) {
    ...
    if (len >= 10) {
        ...
        if (ebcdic_indicators >= 2) {
            return TAURUS_ENCODING_EBCDIC_037;
        }
    }
    ...
    if (high_bit_count > 0) {
        return TAURUS_ENCODING_ISO8859_1;
    }
 ...
 */
taurus_encoding_t taurus_encoding_detect(const char* data, size_t len) {
    if (!data || len == 0) {
        return TAURUS_ENCODING_UNKNOWN;
    }
    ...
    taurus_encoding_t encoding = detect_bom(udata, len);
    if (encoding != TAURUS_ENCODING_UNKNOWN) {
        return encoding;
    }
 ...
 */
const char* taurus_encoding_name(taurus_encoding_t encoding) {
    if (encoding < 0 || encoding >= TAURUS_ENCODING_UNKNOWN) {
        return NULL;
    }
 ...
 */
char* taurus_encoding_convert(const char* from_encoding,
 ...
 */
char* taurus_encoding_to_utf8(taurus_encoding_t from_encoding,
                               const char* input,
                               size_t input_len,
                               size_t* output_len) {
    if (!input || !output_len) {
    ...
    /* Get encoding name */
    const char* from_name = taurus_encoding_name(from_encoding);
    if (!from_name) {
        return NULL;
    }

    /* If already UTF-8, just copy */
    if (from_encoding == TAURUS_ENCODING_UTF8) {
    ...
    /* Convert to UTF-8 */
    return taurus_encoding_convert(from_name, "UTF-8", input, input_len, output_len);
}

/**
 * Parse encoding from XML declaration
 */
char* taurus_encoding_parse_declaration(const char* xml, size_t len) {
 ...
 */
char* taurus_encoding_auto_convert(const char* xml,
                                    size_t len,
                                    size_t* output_len,
    ...
    char* declared_encoding = taurus_encoding_parse_declaration(xml, len);

    /* If no declared encoding, use BOM/heuristic detection */
    taurus_encoding_t detected = TAURUS_ENCODING_UNKNOWN;
    if (!declared_encoding) {
        detected = taurus_encoding_detect(xml, len);
    }

    /* Determine which encoding to use */
    const char* encoding_to_use = declared_encoding;
    if (!encoding_to_use && detected != TAURUS_ENCODING_UNKNOWN) {
        encoding_to_use = taurus_encoding_name(detected);
    }
    ...
    if (detected_encoding) {
        ...
        } else if (detected != TAURUS_ENCODING_UNKNOWN) {
            const char* name = taurus_encoding_name(detected);
    ...
    if (detected == TAURUS_ENCODING_UTF16LE || detected == TAURUS_ENCODING_UTF16BE) {
        utf16_encoding_t encoding = (detected == TAURUS_ENCODING_UTF16LE) ? UTF16_LE : UTF16_BE;
    ...
    char* converted = taurus_encoding_convert(encoding_to_use, "UTF-8",
                                               xml, len, output_len);
    ...
    if (!converted && declared_encoding) {
        ...
        detected = taurus_encoding_detect(xml, len);
        if (detected != TAURUS_ENCODING_UNKNOWN) {
            encoding_to_use = taurus_encoding_name(detected);
            if (encoding_to_use) {
                ...
                converted = taurus_encoding_convert(encoding_to_use, "UTF-8",
                                                   xml, len, output_len);
            }
        }
    }
