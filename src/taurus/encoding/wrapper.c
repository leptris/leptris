 ...
 *
 * Extracted from taurus.c as phase 1 of the file-split roadmap
 ...
 *     here and freed here — `taurus_parse` makes its own pool copy.
...
/* Local alias for the macro defined in taurus_internal.h.  Kept local
...
TAURUS_API TaurusDocument taurus_parse_string_with_encoding(
    const char* xml, size_t length, TaurusStatus* status) {
    ...
    if (bom == UTF16_BOM_LE || bom == UTF16_BOM_BE) {
        ...
        /* taurus_parse() heap-copies its input into doc->xml_buffer;
         * our intermediate is now redundant. */
        WRAPPER_FREE(utf8_buffer);

        if (doc) {
            if (doc->encoding) WRAPPER_FREE(doc->encoding);
            doc->encoding = taurus_strdup((enc == UTF16_LE) ? "UTF-16LE" : "UTF-16BE");
        }
    ...
    char* utf8_xml = taurus_encoding_auto_convert(
    ...
    /* taurus_parse already copied utf8_xml into doc->xml_buffer. */
