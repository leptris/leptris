...
char* xslt_format_number(const XsltStylesheet* sheet, double value,
    ...
    LeptrisNumFormatSpec spec;
    spec.decimal_sep = df ? df->decimal_sep : ".";
    spec.grouping_sep = df ? df->grouping_sep : ",";
    spec.nan = (df && df->nan) ? df->nan : "NaN";
    spec.infinity = (df && df->infinity) ? df->infinity : "Infinity";
    spec.zero_digit = df ? df->zero_digit : '0';
    spec.minus_sign = df ? df->minus_sign : '-';
    return leptris_format_number_core(value, pattern, &spec);
}
...
static void xslt_register_handler(XPathFunctionRegistry* r, const char* name,
    ...
    xpath_function_registry_register_ud(r, name, h, min_args,
                                        max_args, user_data);
}
