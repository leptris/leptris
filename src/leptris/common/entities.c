    ...
    while (input[in_pos]) {
        if (input[in_pos] == '&') {
            ...
            if (strncmp(input + in_pos, "lt;", 3) == 0) {
                ...
                int strict_mode = leptris_get_strict_mode();
                if (input[in_pos] != ';') {
                    if (strict_mode) {
...
char* leptris_decode_entities_view(const struct leptris_string_view* sv, LeptrisMemoryPool* pool) {
    if (!sv || leptris_sv_is_empty(sv)) {
        if (pool) {
            char* empty = (char*)leptris_pool_alloc(pool, 1);
            if (empty) empty[0] = '\0';
            return empty;
        }
        char* empty = malloc(1);
    ...
    char* output;
    if (pool) {
        output = (char*)leptris_pool_alloc(pool, sv->length + 1);
    } else {
...
char* leptris_decode_entities_with_dtd(const char* input, const LeptrisDTD* dtd) {
    ...
    if (dtd) {
        size_t result_len = 0;
        char* result = leptris_dtd_expand_entities(dtd, input, strlen(input), &result_len);
        return result;
    }

    /* Otherwise, fall back to predefined entities only */
    return leptris_decode_entities(input);
}
...
char* leptris_decode_entities_view_with_dtd(const struct leptris_string_view* sv,
    ...
    if (dtd) {
        ...
        size_t result_len = 0;
        char* result = leptris_dtd_expand_entities(dtd, temp_buf, sv->length, &result_len);
        free(temp_buf);

        if (result && pool) {
            /* Copy result to pool */
            char* pooled = (char*)leptris_pool_alloc(pool, result_len + 1);
            if (pooled) {
    ...
    /* Otherwise, fall back to predefined entities only */
    return leptris_decode_entities_view(sv, pool);
}
