...
uint8_t leptris_chartype_table[256] = {
    /* ASCII letters */
 ...
 * LEPTRIS_CONSTRUCTOR (GCC/Clang: constructor attribute; MSVC: CRT
 * initializer section). This is cheaper than enumerating 128
 * designated initializers and keeps the table human-readable. */
static void leptris_chartype_init(void) {
    for (int i = 0x80; i < 0x100; i++) {
        ((uint8_t*)leptris_chartype_table)[i] |= CT_UTF8;
    }
}

LEPTRIS_CONSTRUCTOR(leptris_chartype_init)
