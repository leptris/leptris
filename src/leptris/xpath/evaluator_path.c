 ...
 */
/* Typed-scalar carrier conversion for predicates: the xq driver
 * binds every variable as a sequence carrier, so a numeric atomic
 * rides one marked text node. A bare variable predicate must then be
 * POSITIONAL (number -> position compare), not EBV-of-a-one-item
 * sequence (cbcl-distinct-values-007). Takes ownership of r; when
 * converted, the carrier copy is freed through the result and a
 * fresh NUMBER result returns. NULL propagates. */
struct leptris_xpath_result* leptris_pred_scalar_convert(
    ...
    if (!r || r->type != XPATH_RESULT_NODESET || !r->value.nodeset_value ||
        ...
        XPATH_NODE_TYPE(r->value.nodeset_value->nodes[0]) !=
            LEPTRIS_NODE_TEXT)
        return r;
    const char* c =
        ((XPathTextNode*)r->value.nodeset_value->nodes[0])->content;
    if (!c || c[0] != '\x03' ||
        (c[1] != 'N' && c[1] != 'F' && c[1] != 'D'))
        return r;
    ...
    if (out) out->value.number_value = v;
    return out;
}
...
static int evaluate_predicate_for_node(XPathContext* ctx,
    ...
    pred_result = leptris_pred_scalar_convert(pred_result);
