    ...
    switch (node->type) {
        ...
        case XPATH_AST_OPERATOR: {
            ...
            if (op == XPATH_OP_IF || op == XPATH_OP_FOR ||
                ...
                op == XPATH_OP_LET || op == XPATH_OP_MAP ||
                op == XPATH_OP_CONCAT || op == XPATH_OP_SWITCH ||
                ...
                op == XPATH_OP_LOOKUP ||
                op == XPATH_OP_INLINE_FN || op == XPATH_OP_FN_REF ||
                op == XPATH_OP_DYN_CALL ||
...
LeptrisXPathBytecode* leptris_xpath_compile_ast(XPathASTNode* ast) {
    if (!ast) return NULL;
    LeptrisXPathBytecode* bc = (LeptrisXPathBytecode*)calloc(1, sizeof(*bc));
    if (!bc) return NULL;
