 ...
 *
 * Maintained lazily on LeptrisDocument. The index has:
 ...
 * ~0.5 us (slice + nodeset build). Brings leptris to libxml2 parity
 ...
 */
#ifndef LEPTRIS_DOM_ELEMENT_INDEX_H
#define LEPTRIS_DOM_ELEMENT_INDEX_H

#include "../leptris_internal.h"

typedef struct leptris_element_index_bucket {
    char* name;                  /* Owned, NUL-terminated */
    size_t name_len;
    LeptrisElement* matches;      /* Array of elements with this name */
    size_t count;
    size_t capacity;
...
} LeptrisElementIndexBucket;

typedef struct leptris_element_index_attr_value {
    char* value;                 /* Owned */
    size_t value_len;
    LeptrisElement* matches;      /* Elements with attr name AND this value */
    size_t count;
    size_t capacity;
    /* Preorder position of each match, parallel to matches (TODO
     * 192c): lets subtree-restricted queries window the bucket. */
    size_t* match_positions;
} LeptrisElementIndexAttrValue;

typedef struct leptris_element_index_attr_bucket {
    char* attr_name;             /* Owned, NUL-terminated */
    size_t attr_name_len;
    LeptrisElement* matches;      /* Elements with this attr (any value) */
    size_t count;
    size_t capacity;
    /* Preorder position of each match, parallel to matches (TODO
     ...
     * small. */
    LeptrisElementIndexAttrValue* values;
    size_t value_count;
    size_t value_capacity;
} LeptrisElementIndexAttrBucket;

typedef struct leptris_element_index {
    LeptrisElement* all_elements; /* Flat array in preorder */
    size_t all_count;
    ...
    LeptrisElementIndexBucket* buckets;
    size_t bucket_count;
     ...
     * attribute scanning. */
    LeptrisElementIndexAttrBucket* attr_buckets;
...
#endif /* LEPTRIS_DOM_ELEMENT_INDEX_H */
