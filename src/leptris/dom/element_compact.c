...
static inline void verify_compact_element_size(void) {
    ...
    sizeof(LeptrisElementCompact);  /* Suppress unused warning */
}
 ...
 */
static inline void* get_page_base(LeptrisElementCompact* elem) {
    if (!elem) return NULL;
    ...
    /* The 4 comes from LEPTRIS_COMPACT_ALIGNMENT (4 bytes) */
    return (char*)elem - (page_offset * LEPTRIS_COMPACT_ALIGNMENT);
}
...
LeptrisElementCompact* leptris_element_as_compact(LeptrisElementNode* elem) {
    /* In compact mode, LeptrisElementNode is actually LeptrisElementCompact */
    /* This is just a cast for type safety */
    return (LeptrisElementCompact*)elem;
}

LeptrisElementNode* leptris_compact_as_element(LeptrisElementCompact* compact) {
    /* Convert compact element back to regular element pointer */
    return (LeptrisElementNode*)compact;
}
...
LeptrisElementCompact* leptris_compact_get_parent(LeptrisElementCompact* elem) {
    if (!elem) return NULL;
    ...
    return (LeptrisElementCompact*)leptris_compact_ptr16_decode(&elem->parent, page_base);
}

void leptris_compact_set_parent(LeptrisElementCompact* elem, LeptrisElementCompact* parent) {
    if (!elem) return;
    ...
    leptris_compact_ptr16_encode(&elem->parent, parent, page_base);
}
...
LeptrisElementCompact* leptris_compact_get_first_child(LeptrisElementCompact* elem) {
    if (!elem) return NULL;
    ...
    return (LeptrisElementCompact*)leptris_compact_ptr8_decode(&elem->first_child, page_base);
}

void leptris_compact_set_first_child(LeptrisElementCompact* elem, LeptrisElementCompact* child) {
    if (!elem) return;
    ...
    leptris_compact_ptr8_encode(&elem->first_child, child, page_base);
}

LeptrisElementCompact* leptris_compact_get_last_child(LeptrisElementCompact* elem) {
    if (!elem) return NULL;
    ...
    return (LeptrisElementCompact*)leptris_compact_ptr8_decode(&elem->last_child, page_base);
}

void leptris_compact_set_last_child(LeptrisElementCompact* elem, LeptrisElementCompact* child) {
    if (!elem) return;
    ...
    /* Encode the child pointer as 8-bit offset */
    leptris_compact_ptr8_encode(&elem->last_child, child, page_base);
}
...
LeptrisElementCompact* leptris_compact_get_next_sibling(LeptrisElementCompact* elem) {
    if (!elem) return NULL;
    ...
    return (LeptrisElementCompact*)leptris_compact_ptr8_decode(&elem->next_sibling, page_base);
}

void leptris_compact_set_next_sibling(LeptrisElementCompact* elem, LeptrisElementCompact* sibling) {
    if (!elem) return;
    ...
    leptris_compact_ptr8_encode(&elem->next_sibling, sibling, page_base);
}
 ...
 */
struct leptris_compact_attribute* leptris_compact_get_first_attribute(LeptrisElementCompact* elem) {
    if (!elem) return NULL;
    ...
    return (struct leptris_compact_attribute*)leptris_compact_ptr8_decode(&elem->first_attribute, page_base);
}
 ...
 */
void leptris_compact_set_first_attribute(LeptrisElementCompact* elem, struct leptris_compact_attribute* attr) {
    if (!elem) return;
    ...
    leptris_compact_ptr8_encode(&elem->first_attribute, attr, page_base);
}
 ...
 */
uint8_t leptris_compact_attribute_count(LeptrisElementCompact* elem) {
    if (!elem) return 0;
 ...
 */
struct leptris_compact_attribute* leptris_compact_get_attribute_by_index(LeptrisElementCompact* elem, uint8_t index) {
    if (!elem || index >= elem->attr_count) return NULL;

    /* Walk the linked list to find the attribute at index */
    struct leptris_compact_attribute* attr = leptris_compact_get_first_attribute(elem);
 ...
 */
struct leptris_compact_attribute* leptris_compact_get_attribute_by_name(LeptrisElementCompact* elem, const char* name) {
 ...
 */
int leptris_compact_add_attribute(LeptrisElementCompact* elem,
    ...
    struct leptris_compact_attribute* attr = (struct leptris_compact_attribute*)leptris_pool_alloc(
        pool, sizeof(struct leptris_compact_attribute));
    if (!attr) return -1;
    ...
    /* Add to linked list */
    struct leptris_compact_attribute* first_attr = leptris_compact_get_first_attribute(elem);
    if (!first_attr) {
        /* First attribute */
        leptris_compact_set_first_attribute(elem, attr);
    } else {
        /* Find last attribute and append */
        struct leptris_compact_attribute* last = first_attr;
        while (last->next) {
            last = last->next;
        }
        last->next = attr;
 ...
 */
const char* leptris_compact_get_name(LeptrisElementCompact* elem) {
    if (!elem) return NULL;
    ...
    if (!leptris_sv_is_empty(&elem->name_view)) {
        ...
        if (leptris_element_get_document(elem) && leptris_element_get_pool(elem)) {
            elem->name = leptris_sv_to_cstr_pooled(&elem->name_view, leptris_element_get_pool(elem));
        } else {
            /* Fallback to regular malloc (not ideal for pool-allocated elements) */
            elem->name = leptris_sv_to_cstr(&elem->name_view);
        }
        return elem->name;
    }
 ...
 */
void leptris_compact_set_prefix_view(LeptrisElementCompact* elem, LeptrisStringView prefix_view) {
    if (!elem) return;
 ...
 */
void leptris_compact_set_namespace_uri_view(LeptrisElementCompact* elem, LeptrisStringView uri_view) {
    if (!elem) return;
 ...
 */
uint8_t leptris_compact_child_count(LeptrisElementCompact* elem) {
    if (!elem) return 0;
 ...
 */
LeptrisElementCompact* leptris_compact_get_child(LeptrisElementCompact* elem, uint8_t index) {
    if (!elem || index >= elem->child_count) return NULL;

    /* Walk the child linked list */
    LeptrisElementCompact* child = leptris_compact_get_first_child(elem);
    for (uint8_t i = 0; i < index && child; i++) {
        child = leptris_compact_get_next_sibling(child);
    }
 ...
 */
LeptrisElementCompact* leptris_compact_element_create_with_view(
    LeptrisStringView name_view,
    LeptrisMemoryPool* pool
) {
    if (!pool) return NULL;
    ...
    LeptrisElementCompact* elem = (LeptrisElementCompact*)leptris_pool_alloc(
        pool, sizeof(LeptrisElementCompact));
    if (!elem) return NULL;

    /* Initialize all fields to zero */
    memset(elem, 0, sizeof(LeptrisElementCompact));
    ...
    leptris_compact_header_init(&elem->header, elem, LEPTRIS_NODE_TYPE_ELEMENT);

    /* Store StringViews - ZERO COPY! */
    elem->name_view = name_view;
    elem->prefix_view = leptris_sv_empty();
    elem->namespace_uri_view = leptris_sv_empty();
    ...
    leptris_element_get_document(elem) = NULL;

    /* Initialize all pointers to NULL */
    elem->parent.offset = LEPTRIS_COMPACT_PTR_NULL;
    elem->first_child.offset = LEPTRIS_COMPACT_PTR_NULL;
    elem->last_child.offset = LEPTRIS_COMPACT_PTR_NULL;
    elem->next_sibling.offset = LEPTRIS_COMPACT_PTR_NULL;
    elem->first_attribute.offset = LEPTRIS_COMPACT_PTR_NULL;
 ...
 * @return New compact element, or NULL on failure
 */
LeptrisElementCompact* leptris_element_compact_create(const char* name, LeptrisMemoryPool* pool) {
    if (!name || !pool) return NULL;

    /* Create StringView from C string */
    LeptrisStringView name_view = leptris_sv_from_cstr(name);
    return leptris_compact_element_create_with_view(name_view, pool);
}
 ...
 */
void leptris_element_compact_free(LeptrisElementCompact* elem, LeptrisMemoryPool* pool) {
