    ...
    if (doc->pool) {
        ...
        if (elem) {
            ...
            if (name_copy) {
                ...
                /* #682 register-elision: colon-free names keep a valid
                 * Round-21 namebp backpointer (slot [doc][name]) — the
                 * detached-element resolver never needs the map, and
                 * set_root validates via get_document. Prefixed names
                 * SPLIT in place below, which drops the backpointer —
                 * those still register. */
                if (strchr(name_copy, ':'))
