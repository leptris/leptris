 ...
 * leptris_pool_alloc_node_with_content so oversized PI data doesn't
...
LeptrisPINode* leptris_pi_create(const char* target,
    ...
    LeptrisPINode* node = (LeptrisPINode*)leptris_pool_alloc_node_with_content(
        pool, sizeof(LeptrisPINode), content_size, &content_storage);
    if (!node) return NULL;
