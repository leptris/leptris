 ...
 * types (LeptrisCompactPtr8/16) have been removed — they were never
...
LeptrisCompactOverflowTable* leptris_compact_overflow_table_create(size_t bucket_count) {
    ...
    LeptrisCompactOverflowTable* table = (LeptrisCompactOverflowTable*)malloc(sizeof(LeptrisCompactOverflowTable));
    if (!table) return NULL;

    table->buckets = (LeptrisCompactOverflowEntry**)calloc(bucket_count, sizeof(LeptrisCompactOverflowEntry*));
    if (!table->buckets) {
        free(table);
        return NULL;
    }
...
static LeptrisCompactOverflowEntry* overflow_entry_alloc(
    LeptrisCompactOverflowTable* table) {
    if (table->slab_remaining == 0) {
        ...
        char* slab = (char*)malloc(OVERFLOW_SLAB_ENTRIES *
                                   sizeof(LeptrisCompactOverflowEntry));
        if (!slab) return NULL;
        table->slabs[table->slab_count++] = slab;
        table->slab_next = (LeptrisCompactOverflowEntry*)slab;
...
void leptris_compact_overflow_table_destroy(LeptrisCompactOverflowTable* table) {
    if (!table) return;
...
static int overflow_table_grow(LeptrisCompactOverflowTable* table) {
    size_t new_count = table->bucket_count * 2;
    LeptrisCompactOverflowEntry** nb = (LeptrisCompactOverflowEntry**)
        calloc(new_count, sizeof(LeptrisCompactOverflowEntry*));
    if (!nb) return -1;   /* stay at current size: correct, slower */
    for (size_t i = 0; i < table->bucket_count; i++) {
        LeptrisCompactOverflowEntry* e = table->buckets[i];
        while (e) {
            LeptrisCompactOverflowEntry* next = e->next;
...
int leptris_compact_overflow_set(LeptrisCompactOverflowTable* table,
                                const void* key,
                                void* value,
    ...
    LeptrisCompactOverflowEntry* entry = table->buckets[index];
    while (entry) {
        if (entry->key == key) {
            entry->value = value;
            entry->doc = doc;
...
void* leptris_compact_overflow_get(LeptrisCompactOverflowTable* table,
                                  const void* key) {
...
void leptris_compact_cleanup(void) {
    if (g_overflow_table) {
        leptris_compact_overflow_table_destroy(g_overflow_table);
...
static LEPTRIS_THREAD_LOCAL struct leptris_document* g_current_document = NULL;

void leptris_compact_set_current_document(struct leptris_document* doc) {
    g_current_document = doc;
}

static struct leptris_document* get_current_document(void) {
...
#define LEPTRIS_INT32_OVERFLOW_SENTINEL INT32_MIN
...
int8_t leptris_compact_ptr8_encode(const void* base, const void* target,
                                   int align_log2,
    ...
    if (d & align_mask) {
        ...
        if (table && leptris_compact_overflow_set(table, field_addr,
                                                  (void*)target, doc) == 0) {
            return LEPTRIS_COMPACT_PTR8_OVERFLOW;
        }
        return 0;
    }
...
void* leptris_compact_ptr8_decode(const void* base, int8_t off,
                                  int align_log2,
                                  const int8_t* field_addr) {
    if (off == 0) return NULL;
    if (off == LEPTRIS_COMPACT_PTR8_OVERFLOW) {
        LeptrisCompactOverflowTable* table = get_overflow_table();
        if (table) return leptris_compact_overflow_get(table, field_addr);
        return NULL;
    }
...
int16_t leptris_compact_ptr16_encode(const void* base, const void* target,
                                     int align_log2,
    ...
    if (d & align_mask) {
        ...
        if (table && leptris_compact_overflow_set(table, field_addr,
                                                  (void*)target, doc) == 0) {
            return LEPTRIS_COMPACT_PTR16_OVERFLOW;
        }
        return 0;
    }
    ptrdiff_t scaled = d >> align_log2;
...
void* leptris_compact_ptr16_decode(const void* base, int16_t off,
                                   int align_log2,
                                   const int16_t* field_addr) {
    if (off == 0) return NULL;
...
void leptris_compact_header_init(LeptrisCompactHeader* header,
                                void* page_base,
                                uint8_t flags) {
    ptrdiff_t offset = (char*)header - (char*)page_base;
    assert(offset % LEPTRIS_COMPACT_ALIGNMENT == 0);
    assert(offset < (256 * LEPTRIS_COMPACT_ALIGNMENT));
    header->page_offset = (uint8_t)(offset >> LEPTRIS_COMPACT_ALIGNMENT_LOG2);
    header->flags = flags;
}

void* leptris_compact_header_get_page(const LeptrisCompactHeader* header,
                                     void* page_base) {
    ptrdiff_t offset = header->page_offset << LEPTRIS_COMPACT_ALIGNMENT_LOG2;
