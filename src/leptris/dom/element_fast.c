...
typedef struct leptris_document leptris_document_struct;
...
static LEPTRIS_THREAD_LOCAL LeptrisDocument g_fast_doc = NULL;

/* ============================================================================
 * Element Creation
 * ============================================================================ */

LeptrisElementFast* leptris_element_fast_create(const char* name, LeptrisDocument doc) {
    if (!name || !doc) return NULL;

    /* Get memory pool from document */
    LeptrisMemoryPool* pool = doc->pool;
    if (!pool) return NULL;
    ...
    size_t total_size = sizeof(LeptrisElementFast) + name_len + 1;
    void* memory = leptris_pool_alloc(pool, total_size);
    if (!memory) return NULL;

    /* Set up element structure */
    LeptrisElementFast* elem = (LeptrisElementFast*)memory;
    char* name_storage = (char*)memory + sizeof(LeptrisElementFast);

    /* Copy name */
    memcpy(name_storage, name, name_len + 1);

    /* Initialize fields */
    elem->type = LEPTRIS_NODE_TYPE_ELEMENT;
...
void leptris_element_fast_append_child(LeptrisElementFast* parent, LeptrisElementFast* child) {
...
void leptris_element_fast_prepend_child(LeptrisElementFast* parent, LeptrisElementFast* child) {
    if (!parent || !child) return;
...
void leptris_element_fast_insert_after(LeptrisElementFast* sibling, LeptrisElementFast* child) {
...
void leptris_element_fast_insert_before(LeptrisElementFast* sibling, LeptrisElementFast* child) {
    if (!sibling || !child) return;
...
void leptris_element_fast_remove_child(LeptrisElementFast* parent, LeptrisElementFast* child) {
    if (!parent || !child) return;
...
void leptris_element_fast_set_name(LeptrisElementFast* elem, const char* name) {
    if (!elem || !name) return;
    ...
    if (elem->parent) {
        ...
        LeptrisElementFast* current = elem->parent;
        while (current && !doc) {
    ...
    LeptrisMemoryPool* pool = g_fast_doc->pool;
    if (!pool) return;

    /* Allocate new name */
    size_t name_len = strlen(name);
    char* new_name = (char*)leptris_pool_alloc(pool, name_len + 1);
    if (!new_name) return;
...
LeptrisElementFast* leptris_element_fast_copy(LeptrisElementFast* source, LeptrisDocument doc) {
    if (!source || !doc) return NULL;

    /* Create new element with same name */
    LeptrisElementFast* copy = leptris_element_fast_create(source->name, doc);
    if (!copy) return NULL;
...
void leptris_element_fast_free(LeptrisElementFast* elem, LeptrisDocument doc) {
...
void leptris_element_fast_set_document(LeptrisDocument doc) {
    g_fast_doc = doc;
}

LeptrisDocument leptris_element_fast_get_document(void) {
    return g_fast_doc;
}

/* Reset fast document (call when document is freed to prevent use-after-free) */
void leptris_element_fast_reset_document(void) {
    g_fast_doc = NULL;
}
