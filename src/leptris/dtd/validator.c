...
int leptris_content_model_match(const char* model, const char* elem_name,
 ...
 * result in O(1).
 *
 * The cache lives for the duration of a single leptris_dtd_validate
...
static void set_error(LeptrisDTDError* error, const char* msg, const char* elem_name) {
    if (!error) return;
...
static int validate_element_recursive(LeptrisElement elem, LeptrisDTD* dtd,
                                       LeptrisDTDError* error,
...
static int validate_element_recursive(LeptrisElement elem, LeptrisDTD* dtd,
            ...
            if (child_count > 0) {
                ...
                if (child_names) {
                    ...
                    if (hit) {
                        ...
                        ok = leptris_content_model_match(
    ...
    leptris_hash_table_for_each((StringHashTable*)dtd->tables.attributes,
                               attr_check_iter, &ctx);
    ...
    for (LeptrisAttribute attr = leptris_element_first_attribute(elem);
        ...
        if (strcmp(ad->attr_type, "ID") == 0) {
            ...
            if (leptris_hash_table_get(id_table, id_value, id_len) != NULL) {
                char msg[160];
                snprintf(msg, sizeof(msg),
            ...
            if (!leptris_hash_table_set(id_table, id_value, id_len,
                                        (void*)(uintptr_t)1,
                                        (LeptrisMemoryPool*)dtd->pool)) {
...
static int validate_idref_pass(LeptrisElement elem, LeptrisDTD* dtd,
                                StringHashTable* id_table,
                                LeptrisDTDError* error) {
    if (!elem) return 1;
    const char* name = leptris_element_name(elem);
    if (name) {
        for (LeptrisAttribute attr = leptris_element_first_attribute(elem);
            ...
            if (strcmp(ad->attr_type, "IDREF") == 0) {
                ...
                if (leptris_hash_table_get(id_table, ref, ref_len) == NULL) {
                    char msg[180];
                    snprintf(msg, sizeof(msg),
                ...
                while (*p) {
                    ...
                    if (leptris_hash_table_get(id_table, start, one_len) == NULL) {
                        char ref_buf[128];
...
int leptris_dtd_validate(LeptrisDocument doc, LeptrisDTD* dtd, LeptrisDTDError* error) {
    if (!doc || !dtd) {
        if (error) {
            set_error(error, "NULL document or DTD passed to leptris_dtd_validate", NULL);
        }
        return -1;
    }
    ...
    StringHashTable* id_table = leptris_hash_table_create(
        (LeptrisMemoryPool*)dtd->pool, 16);
    if (!id_table) {
    ...
    leptris_hash_table_destroy(id_table);
    return rc;
}
