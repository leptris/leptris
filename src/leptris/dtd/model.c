...
DTDAttributeDecl* dtd_attribute_decl_create_pooled(const char* element,
    ...
    DTDAttributeDecl* decl = (DTDAttributeDecl*)leptris_pool_calloc(pool, sizeof(DTDAttributeDecl));
    if (!decl) return NULL;

    /* Pool-allocate each string so the whole decl is pool-owned. */
    size_t elen = strlen(element);
    char* element_copy = (char*)leptris_pool_alloc(pool, elen + 1);
    if (!element_copy) return NULL;
    memcpy(element_copy, element, elen + 1);
    decl->element_name = element_copy;

    size_t alen = strlen(attr);
    char* attr_copy = (char*)leptris_pool_alloc(pool, alen + 1);
 ...
 * the document's pool, so leptris_document_free releases everything
 * in one leptris_pool_destroy call.
 */
LeptrisDTD* leptris_dtd_create(LeptrisMemoryPool* pool) {
    if (!pool) return NULL;

    LeptrisDTD* dtd = (LeptrisDTD*)leptris_pool_calloc(pool, sizeof(LeptrisDTD));
    if (!dtd) return NULL;
    ...
    dtd->tables.entities   = leptris_hash_table_create(pool, 128);
    dtd->tables.elements   = leptris_hash_table_create(pool, 64);
    dtd->tables.notations  = leptris_hash_table_create(pool, 32);
    dtd->tables.attributes = leptris_hash_table_create(pool, 128);
 ...
 * leptris_pool_destroy when the document is freed.  This function is a
...
void ttdtd_free(LeptrisDTD* dtd) {
    ...
    if (dtd->owns_pool && dtd->pool) {
        leptris_pool_destroy((LeptrisMemoryPool*)dtd->pool);
    }
}
 ...
 * leptris_pool_destroy releases it.  This function is a no-op kept
...
int ttdtd_add_entity(LeptrisDTD* dtd, DTDEntityDecl* entity) {
    ...
    if (leptris_hash_table_set(dtd->tables.entities, entity->name, name_len,
                              entity, dtd->pool)) {
        dtd->entity_count++;
        return 1;
    }
...
DTDEntityDecl* ttdtd_lookup_entity(const LeptrisDTD* dtd, const char* name) {
    ...
    size_t name_len = strlen(name);
    return (DTDEntityDecl*)leptris_hash_table_get(dtd->tables.entities, name, name_len);
}
...
DTDElementDecl* ttdtd_element_create_pooled(const char* name, LeptrisMemoryPool* pool) {
    ...
    DTDElementDecl* element = (DTDElementDecl*)leptris_pool_calloc(pool, sizeof(DTDElementDecl));
    if (!element) return NULL;

    size_t name_len = strlen(name);
    char* name_copy = (char*)leptris_pool_alloc(pool, name_len + 1);
...
int ttdtd_add_element(LeptrisDTD* dtd, DTDElementDecl* element) {
    ...
    /* Add to hash table */
    if (leptris_hash_table_set(dtd->tables.elements, element->name, name_len,
                              element, dtd->pool)) {
        dtd->element_count++;
        return 1;
    }
...
DTDElementDecl* ttdtd_lookup_element(const LeptrisDTD* dtd, const char* name) {
    ...
    size_t name_len = strlen(name);
    return (DTDElementDecl*)leptris_hash_table_get(dtd->tables.elements, name, name_len);
}
...
DTDAttributeDecl* ttdtd_lookup_attribute(const LeptrisDTD* dtd,
    ...
    if (!key) return NULL;
    size_t key_len = strlen(key);
    DTDAttributeDecl* result = (DTDAttributeDecl*)leptris_hash_table_get(
...
int ttdtd_add_notation(LeptrisDTD* dtd, DTDNotationDecl* notation) {
    ...
    /* Add to hash table */
    if (leptris_hash_table_set(dtd->tables.notations, notation->name, name_len,
                              notation, dtd->pool)) {
        dtd->notation_count++;
        return 1;
    }
