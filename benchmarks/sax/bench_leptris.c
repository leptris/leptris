/* sax/bench_leptris.c — Leptris SAX parser benchmarks.
 *
...
static void bench_sax_file(void* ctx) {
    ...
    h.start_element = on_start_element;
    h.characters    = on_characters;
    leptris_sax_parse(f->buf, f->len, &h, &ctr);
    (void)ctr;
}
...
int main(void) {
    ...
    bench_print_header("Leptris SAX");
    ...
    if (large.buf) {
        ...
        bench_print_summary("Leptris SAX", results, 3);
        bench_write_json("leptris", results, 3, "bench_sax_leptris.json");
    } else {
        fprintf(stderr, "note: fixtures/large.xml not found; skipping large SAX benchmark.\n");
        BenchResult results[] = { r1, r2 };
        bench_print_summary("Leptris SAX", results, 2);
        bench_write_json("leptris", results, 2, "bench_sax_leptris.json");
    }
